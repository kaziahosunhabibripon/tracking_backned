...
@Resolver(() => ConversionReportRecord)
export class ConversionReportsResolver {
  ...
  @Query(() => [ConversionReportRecord], { name: 'conversionReports' })
  async list(
  ...
  ): Promise<ConversionReportRecord[]> {
    const result = await this.adminReportsService.conversionReports({
      preset: range?.preset,
      from: range?.from,
      to: range?.to,
      cursor,
