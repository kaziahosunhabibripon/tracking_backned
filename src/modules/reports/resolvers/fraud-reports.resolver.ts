...
@Resolver(() => AffiliateFraudReportRecord)
export class FraudReportsResolver {
  ...
  @Query(() => [AffiliateFraudReportRecord], { name: 'fraudReports' })
  async list(
  ...
  ): Promise<AffiliateFraudReportRecord[]> {
    const result = await this.adminReportsService.fraudReports({
      preset: range?.preset,
      from: range?.from,
      to: range?.to,
      cursor,
