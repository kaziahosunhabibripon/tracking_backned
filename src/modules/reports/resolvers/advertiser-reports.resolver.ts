...
@Resolver(() => AdvertiserReportRecord)
export class AdvertiserReportsResolver {
  ...
  @Query(() => [AdvertiserReportRecord], { name: 'advertiserReports' })
  async list(
  ...
  ): Promise<AdvertiserReportRecord[]> {
    const result = await this.adminReportsService.advertiserReports({
      preset: range?.preset,
      from: range?.from,
      to: range?.to,
      cursor,
