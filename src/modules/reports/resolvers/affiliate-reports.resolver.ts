...
@Resolver(() => AffiliateReportRecord)
export class AffiliateReportsResolver {
  ...
  @Query(() => [AffiliateReportRecord], { name: 'affiliateReports' })
  async list(
  ...
  ): Promise<AffiliateReportRecord[]> {
    const result = await this.adminReportsService.affiliateReports({
      preset: range?.preset,
      from: range?.from,
      to: range?.to,
      cursor,
