...
@Resolver(() => AffiliatePostbackLogRecord)
export class AffiliatePostbackLogsResolver {
  ...
  @Query(() => [AffiliatePostbackLogRecord], { name: 'affiliatePostbackLogs' })
  async list(
  ...
  ): Promise<AffiliatePostbackLogRecord[]> {
    const result = await this.adminReportsService.affiliatePostbackLogs({
      preset: range?.preset,
      from: range?.from,
      to: range?.to,
      cursor,
